...
var Urbanismo = {
    ...
    readEdificioByID: function (params, callback) {
        ...
        var myarr = ["id_edifica", "n_proc", "n_alvara"];
        var arraycontainskey = (myarr.indexOf(params.key) > -1);

        if (arraycontainskey) {
            //var sql = 'SELECT  sensorid, address, location, installdate, sensortype, metric, calibrated, quantity, decimalplaces, cal_a, cal_b, read_interval, record_sample FROM ' + table,
            var sql = "select st_x(st_centroid(the_geom)), st_y(st_centroid(the_geom)) from edificios.edificado_vti2 where " + params.key + " = '" + params.id + "'";
            where = '', order = '', paging = '';
            var detail = global.App.connectionide.split('/');
            pg.connect({
                user: detail[2].split('@')[0].split(':')[0],
            ...
            return dberror('Invalid key', '', err, callback);
        }
    },
